...
on `/v1/capabilities` so a caller knows before it sends. `/healthz` is
...
`/v1/capabilities` so it is never a mystery, and set Q5's budget against the
...
environment — but it is invisible to `/v1/capabilities`, absent from the signed
...
- **Container-scoped**, set by whoever provisions the container, exactly as
  credentials work now. `/v1/capabilities` would grow a report of which
