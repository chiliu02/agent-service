...
| **Implementation** | each `impl/*/pyproject.toml` | the **image tag**, and `capabilities.impl.version` | that build changes, for any reason |
...
`/v1/capabilities` already publishes the same two values as `sdk.name` and
...
| **AS-32** | An extension a client must act on is published on `/v1/capabilities`. Prose is the only difference a client may be required to ignore |
