# Deriving `RunOptions` from `/v1/capabilities`
...
1. **Fetch both documents, once per CONTAINER — not once per build.**
   `GET /v1/capabilities` for values and defaults, `GET /openapi.json` for the
   field descriptions. **`/openapi.json` needs no bearer token**: auth covers
   paths under `/v1` and nothing else, on all three builds.
   **Cache per instance, keyed by the base URL you called, and re-fetch when it
   restarts.** `impl.version` is not a safe cache key on its own: roughly a third
   of the payload comes from the deployment's environment rather than from the
   build, so two containers of one image answer differently (§7.4).
2. **Remove every refused field** with the predicate above. A field refused by
   `types` or `values` is *partly* available — remove only the shapes named.
3. **Fill each surviving control's options from its capability**, never from a
   constant in your own code and never from the OpenAPI enum (§3.1).
4. **Attach the tip** from the OpenAPI property description, falling back to the
   capability's own per-value description (§3.2).
5. **Prefill the default** from the capability, or leave the control empty and
   labelled *server default* — never from a guess (§3.3).
6. **Read the gates separately.** `allow_mcp_servers` and
   `allow_supplied_sdk_session_id` govern a field without appearing in
   `unsupported_options` on two of the three builds (§8).
7. **Bound the numbers with `limits`.** A `max_allowed_*` key is a ceiling on
   what a request may ask for; any other key is a figure the build enforces on
   its own. **An absent key is not a limit of zero — it is no limit published.**
8. **Submit only what the user touched.** Omission means *server default*;
   a prefilled control that always submits converts a default into a pin.

---

## 3. Three sources, three questions

### 3.1 Values — the dropdown comes from `/v1/capabilities`
...
| | Fields |
|---|---|
| **Has a tip** | `model`, `resume`, `system_prompt`, `allowed_tools`, `disallowed_tools`, `setting_sources`, `max_budget_usd`, `workspace_subdir`, `include_raw`, `mcp_servers`, `strict_mcp_config` |
...
configuration — which is what `/v1/capabilities` is for.

| Tier | Source | Fields |
|---|---|---|
| **1** | the capabilities response | `model` ← `default_model`; `max_turns` ← `limits.default_max_turns`; `max_budget_usd` ← `limits.default_max_budget_usd`; `timeout_s` ← `limits.default_request_timeout_s`; `allowed_tools` ← `default_allowed_tools`; `strict_mcp_config` ← `strict_mcp_config`; `workspace_subdir` ← relative to `workspace_dir` |
...
`RunOptions` has sixteen properties. This is every one of them.
...
| `workspace_subdir` | text | — | document | none; show `workspace_dir` as the prefix |
...
3. **`always_disallowed_tools` is a filter, not a rejection.** Naming
   `run_shell_command` in `allowed_tools` on `gemini-python` does not produce a
   `400` — the name is dropped and the session proceeds. The published list is
   the notice; there is no error to catch.
...
- **The three generated pages** —
  ...
  §6's generator, running, against each build's own `/v1/capabilities` example as
