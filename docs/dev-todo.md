...
- [x] **5 · Split the version streams** — **done 2026-08-07, shipped as
      ...
      **The design left one thing open and it needed the user: what
      ...
      - **`test_the_advertised_version_matches_the_package_version` was
        ...
        `deployment.service.impl.version` == `pyproject.toml`.
...
- [x] **6 · Core/extension split of the document — DONE, in a different shape
      ...
      **It inverts what the step assumed.** The delta against
      ...
      - **AS-32 — every behavioural extension is published on
        `/v1/deployment`** (user, 2026-08-08). This generalises what 0.18.0
...
- [x] **7 · The second implementation — DONE, and there are now THREE**
      ...
      **Two of the bullets below were closed the same day and are corrected
      ...
      - [x] ~~**No MCP**~~ — **built 2026-08-09, and measured through `/v1`**: a
        ...
        **Two limits, published as `deployment.accepts.mcp` rather than left to a
      ...
      - [x] ~~**`setting_sources` is refused**~~ — **honoured 2026-08-09**, and
        ...
        **Measured yes** — Codex reads `AGENTS.md` from the thread's cwd and
        ...
        value. **`deployment.accepts.setting_sources` already existed and this build
      ...
      - [x] ~~**The paid conformance tier could not run against this build at
        ...
        Replaced by a `cheap_options` fixture built from `/v1/deployment`,
      ...
      - [x] ~~**`default_model: gpt-5-codex` 404s on `/v1/responses`**~~ —
        ...
        **The default moved to `gpt-5.1`** (user, 2026-08-10). The first answer
        ...
        remain the levers, and `deployment.config.default_model` publishes the value
...
- ~~**`PermissionMode` is a union of two vendors' spellings**~~ — **built
  ...
  **all six values are `claude_agent_sdk`'s own enum**, adopted as the
  ...
  on `/v1/deployment`, `permission_mode` is an opaque string, and a build
...
just `deployment.accepts.permission_modes` changing from `[string]` to `[{id, name,
...
`/v1/deployment`. Studio offered it as a nicety; folding it in would stop this
...
`auth_required` published on `/healthz` and `/v1/deployment`.
...
- [x] **Multi-reference directories, verified in a container.** Two mounted and
      listed: boots, `/v1/deployment` reports both, both readable. Second
...
shipped the same day (`/v1/deployment` gains `credential_sources`,
...
## Carry into the `0.21.0` release notes (2026-09-03)

**Two breaking changes and three fixed defects, all on the request surface.**
They are cheap to write up now and expensive to reconstruct at the cut.

1. **`workspace_subdir` is renamed `working_directory`, with no alias.** Sending
   the old name is now a **422 naming the key** (item 5), so the rename fails
   loudly rather than silently — but it still fails, and that is what the notice
   is for.
2. **`RunOptions.working_directory` is now validated on `codex-python`**
   (`CX-64`) and **honoured on `gemini-python`** (`GP-68`), where it had been
   read by nothing. On gemini it is also a boundary: the agent's guard is
   anchored at the working directory.
3. **`include_raw` is honoured on `gemini-python`** (`GP-67`) — it was accepted
   and ignored, so responses carried the agent's whole payload whatever was
   asked. The recorded transcript still keeps `raw`.
4. **Field descriptions across `RunOptions` and `Capabilities`**, including the
   three-layer tool story and the "an unknown tool name is dropped, not refused"
   warning. Twenty-eight published fields had no description at all.

5. **An unknown `RunOptions` property is now refused** (user, 2026-09-03), as a
   **422** carrying `loc` and `type: extra_forbidden` rather than a 400: an
   unrecognised key was never understood, which is the framework's validation
   channel, while a 400 means understood-and-declined and belongs to
   `unsupported_options`. `RunOptions` sets `extra="forbid"` in the shared model,
   so all three builds changed at once, and `spec/conformance/` carries the
   clause. **Breaking for any caller sending a stray key.**

## Also for the `0.21.0` notes: the payload was regrouped and the route renamed (2026-09-03)

**Breaking, and it is the first call every client makes.**

5. **`GET /v1/capabilities` is `GET /v1/deployment`**, and the old route is gone
   rather than redirected. Half of what it carried was never a capability —
   `workspace_dir` and `max_sessions` are configuration — which is what made the
   name mislead.
6. **The payload is four groups**: `service` (who is answering), `config` (how
   this instance is set up), `accepts` (what you may send), `behaviour` (what it
   does and reports). Every field moved; none was removed.
7. **`limits` and `mcp` were cut along that line.** `accepts.limits` holds
   `default_*`/`max_allowed_*`, `behaviour.limits` holds what the service
   enforces; the tool-call timers are `behaviour.mcp_tool_call`. A client
   reading `limits.session_idle_ttl_s` or `mcp.tool_call` must move.
8. **New: `GET /v1/schemas/run-options`** — `accepts` rendered as JSON Schema
   2020-12 for validation or form generation. Additive.

**The migration is mechanical and the notice matters more than the diff**: a
client that misses it gets a `404` on its first call, which is at least loud.
The half that is not loud is `limits` — reading a key that moved returns
