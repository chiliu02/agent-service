...
| `sandbox.network_access` | true | **false** (bubblewrap) — **the agent's SHELL only.** Its hosted web tool searches and fetches pages regardless, on by default, with no switch this service controls (measured 2026-09-02, `CX-63`) | true |
...
|---|---|---|---|
| nested read-only mask | **held** | **held** | **held** |
| unmasked write (the control) | writable | writable | writable |
| `/workspace/.git` | **writable** | **read-only** | **writable** |
| `/workspace` mount lines seen by the agent | 2 — what Docker created | **7** | 2 — what Docker created |
...
`RunOptions` has sixteen fields. **Five transfer cleanly and are worth naming
first, because a uniform field is a result rather than an absence:** `model`,
`resume`, `timeout_s`, `include_partial_messages` and `include_raw` mean the same
thing on all three. `timeout_s` is uniform in *effect* because this service
enforces it rather than any SDK — though the two builds check its ceiling at
different moments, below — and `resume` is uniform despite three unrelated
mechanisms underneath: an SDK session id, a `thread_resume(thread_id)`, and a
`--session-file`.

**`include_raw` joined that list on 2026-09-03** and `working_directory` left it
the same day, both by measurement rather than by design change. The eleven that
diverge:
...
**State it once, plainly: on all three builds, the agent reads configuration
...
| | claude-python | codex-python | gemini-python |
|---|---|---|---|
| **SUPPLY it by request** — send memory, skills, subagents, commands, plugins, hooks | **no** | **no** | **no** |
| **SUPPRESS what is on disk** | **yes, fully** — `setting_sources: []`, and it is the server default (measured, `CP-060`) | **partly** — `setting_sources` without `project` suppresses `AGENTS.md`; `CODEX_HOME`'s own `config.toml` is read whatever you send, and `local` is a 400 | **no** — `setting_sources` is refused outright; a `GEMINI.md` or `.gemini/settings.json` in the mounted workspace is read on every turn |

**Per kind, and the empty column is the point:**

| Ambient input | claude-python | codex-python | gemini-python |
|---|---|---|---|
| Memory file (`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`) | on/off only | on/off only (project layer) | **neither** |
| Skills | **neither** — the SDK's `skills` is a name filter over what is on disk, never a definition | no concept | **neither** — `${AgentSkills}` interpolates what is on disk |
| Subagents | **neither** by API. The SDK accepts inline definitions; no `RunOptions` field carries them | no concept | **neither** |
| Slash commands, output styles, plugins | **neither** | **neither** (`CODEX_HOME` prompts) | **neither** |
| Hooks | **neither** — server-side only, via `permission_enforcement` | n/a | n/a |
| Settings files (permissions, env) | partly — `allowed_tools`, `disallowed_tools`, `permission_mode`; scoped rules are not enforced | governed by sandbox instead | `allowed_tools` generates the policy file, which IS the boundary |
| MCP servers | **fully** — `mcp_servers`, and `strict_mcp_config: true` (the default here) shuts out `.mcp.json` | **fully** | **fully** — and `--allowed-mcp-server-names` is the one channel the workspace cannot override (`GP-47`) |

**MCP is the exception that proves the rest.** It is the one ambient input a
caller can both supply and suppress on every build, and it got there by being
built that way deliberately — because a repository registering a server means a
subprocess nobody asked for. Every other row is the same hazard with no such
control.
...
all three builds, and nothing in an OpenAPI document would have told it.
