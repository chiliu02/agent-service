...
(`GET /v1/deployment` reports `impl.name` and `impl.version`; the image
...
- An unauthenticated caller reaches the API. Authentication is **off by
  ...
  and `/v1/deployment` says so via `auth_required`.
...
- Anything that contradicts what `/v1/deployment` publishes. The published
...
7. **Read `always_disallowed_tools`** on `/v1/deployment` before assuming a
