...
This module owns three responsibilities the SDK will not do for us:
  ...
  2. Keep working_directory inside the workspace root.
...
class McpServersNotAllowedError(ValueError):
    ...
    `deployment.config.allow_mcp_servers` publishes this before it is hit, the same
...
def build_options(
    ...
    effective_cwd = resolve_workspace(settings, req.working_directory)
