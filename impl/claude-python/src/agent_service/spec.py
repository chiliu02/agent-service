...
def specification() -> dict[str, object]:
    ...
    try:
        service_version = version("agent-service-claude-python")
    except PackageNotFoundError:  # pragma: no cover - only outside an install
        service_version = None
    return {
        # THIS BUILD's version, read from the installed package. It was the
        # only version there was until 0.12.0; `document_version` beside it is
        # the other half of the split Plan 8 step 5 made, and is added here for
        # the same reason the whole module exists -- a caller provisioning
        # containers can ask which interface an image implements without
        # starting it.
        "version": service_version,
        "document_version": DOCUMENT_VERSION,
        # **The OTHER thing this image is built against** (CP-147). An image
        ...
        "impl": {"name": IMPLEMENTATION_NAME, "version": service_version},
        "credential_sources": list(CREDENTIAL_ENV_VARS),
        # The agent target this build drives -- `claude`, the family name
        # without the language suffix (CP-146). A consumer relaying to a vendor
        # maps it: `claude` is the Anthropic API.
        "model_api": MODEL_API,
        "provider_selectors": list(PROVIDER_SELECTOR_ENV_VARS),
        # 0.19.0, both asked for by the consumer and both accepted in the shape
        # and under the names it proposed.
        #
        # **`auth_enforced` is about the BINARY; `auth_required` is about the
        # instance.** The second is already on `/healthz` and
        # `/v1/capabilities` and means *a token is configured here*. This one
        # means *this image checks the header at all*, and the consumer needs it
        # BEFORE it decides whether to set a token -- which is a decision made
        # while provisioning, when nothing is running to ask.
        "auth_enforced": True,
        # The variable that moves this build's model endpoint. **Singular**: no
        # build here needs a list, and if one ever does the consumer takes the
        # first entry.
        #
        # Measured rather than assumed -- this side has run turns against a
        # redirected host and against a black-hole listener inside the
        # container. The Codex build publishes `OPENAI_BASE_URL` and reads it
        # itself, because nothing in that app-server reads any variable at all.
        "endpoint_source": ENDPOINT_ENV_VAR,
        # The variable an additional certificate authority is delivered under,
        # asked for by the consumer after a private terminator cost it a turn
        # (CP-144). **The third variable of this kind and the same question as
        # the other two**: a fact about the runtime inside this image that
        # nothing on its surface reveals.
        #
        # **Absent and null differ**, as they do for `llm_correlation`: an image
        # too old to publish the field has never been measured, `null` means
        # measured and there is none. This build publishes an object because the
        # name alone is not enough to act on -- a directory where a file is
        # wanted fails exactly as a wrong name does, and whether the default
        # trust store SURVIVES decides whether a container can still reach a
        # public host.
        "ca_bundle_source": CA_BUNDLE_SOURCE,
        # 0.13.0, and it is here rather than on `/v1/capabilities` BECAUSE OF
        # WHEN IT IS NEEDED. A caller reads this to decide how to reach a
        # container; by the time `/v1/capabilities` can answer, the caller has
        # already connected and the value could only ever agree with the socket
        # it connected on, or contradict it. Same argument that put
        # `credential_sources` in this module: the question is asked BEFORE the
        # service exists.
        #
        # `address` is a REQUIREMENT of the contract -- every implementation
        # listens on all IPv4 interfaces and none binds loopback, so there is
        # nothing here for a caller to branch on. `port` is a VALUE, and is
        # published precisely because a second implementation in another
        # language may reasonably choose a different one. Freezing 8000 into
        # the contract would bake a detail of this image into the platform's
        # interface; publishing it lets a consumer look it up instead of
        # hardcoding it.
