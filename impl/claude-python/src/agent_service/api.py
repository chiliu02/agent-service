...
def create_app(
    ...
    def _capabilities_payload(config) -> Capabilities:
        ...
        return Capabilities(
            ...
            limits={
                ...
                #
                # Every other figure here bounds a REQUEST; this one bounds how
                # long the service keeps something. It sits here anyway because
                # `limits` is what a caller already reads per container, and a
                # second object for one number would be a shape nobody has a
                # use for.
            ...
            mcp={
                ...
                # `sse`/`http` one because `stdio` is the more generous of the
                # two and the published value never is, and the hard cap is 1e8
                # milliseconds to the second.
                "tool_call": {
    ...
    @app.get("/v1/capabilities", response_model=Capabilities, tags=["meta"])
    async def capabilities(config: SettingsDep) -> Capabilities:
