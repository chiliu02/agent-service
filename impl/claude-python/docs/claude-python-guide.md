...
| `GET` | `/v1/capabilities` | Resolved defaults, vocabularies, and what this build *requires* |
...
- **`info.version` is the DOCUMENT's version, not the build's.** Read
  `capabilities.impl.version` for the build and `capabilities.spec.document_version`
...
- **Never branch on which image you have.** Anything a client must act on is on
  `GET /v1/capabilities` (AS-32). If you find yourself diffing two documents or
...
`capabilities.sandbox` reports `network_access: true` and
...
| Situation | You get |
|---|---|
| more than `max_sessions` open at once | **429** — `GET /v1/sessions` is the numerator, `capabilities.max_sessions` the denominator |
...
`capabilities.allow_supplied_sdk_session_id` first.
...
**7. Reading `/v1/capabilities` is yours.** Every difference between builds and
