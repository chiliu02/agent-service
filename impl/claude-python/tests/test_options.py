...
def test_workspace_subdir_is_resolved_under_the_root(settings: Settings) -> None:
...
def test_workspace_subdir_cannot_escape(settings: Settings, bad: str) -> None:
    with pytest.raises(InvalidWorkspacePath):
        resolve_workspace(settings, bad)


def test_workspace_subdir_that_does_not_exist_is_rejected(settings: Settings) -> None:
...
async def test_permission_enforcement_hook_effective_cwd_is_the_workspace_subdir(
    tmp_path: Path,
) -> None:
    ...
    opts, _ = build_options(RunOptions(workspace_subdir="proj"), s)
