...
def test_permission_mode_is_NO_LONGER_constrained_by_the_schema() -> None:
    ...
    honour. Each build declares its own set on `/v1/capabilities` now, so the
...
def test_capabilities_instantiation() -> None:
    caps = Capabilities(
        ...
        # `tool_call` is REQUIRED and `server_name_pattern` is not, which is the
        # same distinction the rest of this payload draws: a build that enforces
        # no name pattern has nothing to say, while a build that cannot state
        # what ends a long tool call must fail to construct this rather than
        # leave a client to discover it by holding one open until it dies.
        mcp={
    ...
    assert caps.sdk.name == "claude-agent-sdk"
    assert caps.sdk.version == "1.0.0"
    assert caps.sdk_version == "1.0.0"
    assert caps.default_model == "claude-sonnet-5"
