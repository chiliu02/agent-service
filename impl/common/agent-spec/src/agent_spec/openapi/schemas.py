...
class RunOptions(BaseModel):
    ...
    **An unknown property is REJECTED, not ignored** (2026-09-03). It arrives as
    the framework's own `422` naming the offending key in `loc` with
    `type: extra_forbidden`, which is the same channel a wrong type already uses.

    The reason is this platform's oldest rule pointed at itself: an option
    accepted and never applied is the defect it treats as its worst, and an
    unknown property is that defect in its purest form -- the caller believes
    they configured something and nothing did. Ignoring it silently is how
    `workspace_subdir` would have survived its own rename.
    """
    ...
    system_prompt: str | dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "**The string REPLACES the agent's own framing** -- safety rules, "
            "tool protocol, workflows. The preset object is the only form that "
            "keeps them and appends, and not every build has one: check "
            '`unsupported_options` for `{field: system_prompt, types: ["object"]}`.\n\n'
            "**Not a substitute for ambient configuration**: it does not become "
            "a memory file or register a skill, and suppresses nothing on disk -- "
            "on one build the workspace's context files are appended after it."
        ),
    )
    allowed_tools: list[str] | None = Field(
        default=None,
        description=(
            "Tools this session may use, in this build's own names -- "
            "`default_allowed_tools` lists the published ones.\n\n"
            "**An unrecognised name is ignored, never refused.** `Reed` for "
            "`Read` grants nothing and reports nothing.\n\n"
            "Names come from three places, and only the first can be published: "
            "**built-ins** (fixed per build and platform, not per model); **MCP "
            "tools** from the servers you send, in that build's spelling "
            "(`mcp__<server>__<tool>`); **skills, subagents and plugins** the "
            "agent reads from the container's disk.\n\n"
            "**Grants permission, not visibility**: an ungranted tool is still "
            "offered to the model, attempted, and denied -- which costs a turn. "
            "`disallowed_tools` removes it. Not a path or command restriction."
        ),
    )
    disallowed_tools: list[str] | None = Field(
        default=None,
        description=(
            "Removes a tool outright. Where `allowed_tools` grants permission "
            "without hiding anything, this is the field that takes a capability "
            "away.\n\n"
            "**An unrecognised name is ignored here too**, so a misspelled "
            "denial denies nothing -- the more dangerous direction of the same "
            "behaviour. Everything in `always_disallowed_tools` is added "
            "whatever you send."
        ),
    )
    permission_mode: PermissionMode | None = Field(
        ...
        description=(
            ...
            "**The ids differ per build** -- read `permission_modes[].id` for "
            "this one's, and `.description` for what each does here. An id it "
            "does not publish is a 400.\n\n"
            "**Not a boundary on any build.** What confines the agent is the "
            "container, the sandbox and (on one build) a generated tool policy: "
            "see `permission_enforcement` and `sandbox`."
        ),
    )
    effort: EffortLevel | None = Field(
        ...
        description=(
            ...
            "Vocabulary is `effort_levels[]`; **empty means the build has no "
            "equivalent and refuses the field with a 400**. A build publishes "
            "only the levels it delivers exactly -- one maps a higher level down "
            "and omits it rather than letting you believe it was honoured."
        ),
    )
    setting_sources: list[SettingSource] | None = Field(
        default=None,
        description=(
            "Which ambient on-disk configuration the agent loads. Server default "
            "is none; a build with no such concept publishes `[]` and refuses "
            "the field.\n\n"
            "**A switch over what is already on the container's disk -- never a "
            "way to send it.** Memory files, skills, subagents, commands, "
            "plugins and settings are read inside the container, and no "
            "`RunOptions` field on any build can supply them. This is the only "
            "lever, it is coarse, and one build does not have it at all.\n\n"
            "Treat that disk as part of the deployment, not part of the request."
        ),
    )
    max_turns: int | None = Field(
        default=None, ge=1,
        description=(
            "Bound on the agent's turns within one request.\n\n"
            "Enforced only where `limits.default_max_turns` and "
            "`max_allowed_turns` are published; elsewhere it is a 400 rather "
            "than accepted and ignored. **Above the ceiling is a 400, never a "
            "clamp** -- a silently shortened budget produces a stop you cannot "
            "explain."
        ),
    )
    ...
    timeout_s: int | None = Field(
        default=None, ge=1,
        description=(
            "Wall clock for this request, in seconds; expiry answers **504**.\n\n"
            "Enforced by this service rather than by any agent SDK, so it means "
            "the same on every build. The ceiling is "
            "`limits.max_allowed_timeout_s` and asking for more is a 400, not a "
            "clamp -- **checked at session creation on one build and per turn on "
            "another**, so a value accepted at create can still be refused later."
        ),
    )
    working_directory: str | None = Field(
        default=None,
        description=(
            "Where the agent starts work. Relative paths the agent uses resolve "
            "from here, and it decides which project-level configuration the "
            "session picks up -- a memory file or a settings file is found "
            "relative to this directory.\n\n"
            "**A path RELATIVE to `capabilities.workspace_dir`** -- `src/api`, "
            "not `/workspace/src/api`. It must stay under the root (`..` is "
            "refused) and **must already exist**: the directory is never created "
            "from request input.\n\n"
            "**Not confinement on most builds** -- the mount and the container "
            "are the boundary, and a working directory restricts nothing a tool "
            "may reach. On one build the agent's own guard is anchored here, so "
            "it also narrows what its file tools can open.\n\n"
            "**Renamed from `workspace_subdir`**, which is now an unknown "
            "property and therefore a 422 naming the key -- not a silent no-op."
        ),
    )
    include_partial_messages: bool = Field(
        default=False,
        description=(
            "Emit output as it is produced rather than only when a message is "
            "complete -- on the SSE routes, token-level deltas instead of one "
            "event per message.\n\n"
            "Accepted by every build, and **the one option whose default comes "
            "from this schema** rather than from server config, so `false` is the "
...
class QueryRequest(BaseModel):
    prompt: str = Field(
        min_length=1, examples=["List the files in this directory"],
        description=(
            "What to ask the agent -- **the user turn, not a system prompt**. "
            "Framing goes in `options.system_prompt`, which on most builds "
            "replaces the agent's own.\n\n"
            "The agent also reads configuration from the container's disk, so "
            "this is never the whole of what it was told."
        ),
    )
    options: RunOptions = Field(
        default_factory=RunOptions,
        description=(
            "Per-request overrides. **Omitting a field is not the same as "
            "sending its default**: an omitted field takes the server's, which "
            "`/v1/capabilities` publishes and which a deployment can change. "
            "Send only what you mean to pin.\n\n"
            "**An unknown property is a 422 naming the key** -- a typo or a "
            "renamed field is reported rather than ignored."
        ),
    )
...
class AgentEvent(BaseModel):
    ...
    seq: int = Field(
        description=(
            "Position within THIS turn, increasing from zero. **Order by it; do "
            "not count with it and do not compare it across turns** -- it is an "
            "index into one turn's event list, not a cursor over the session."
        )
    )
    ...
    subtype: str | None = Field(
        default=None,
        description=(
            "The originating agent's own sub-kind for this event -- `init` on a "
            "system frame, for example.\n\n"
            "**Another vendor's vocabulary, passed through.** Useful for display "
            "and for debugging; **branch on `type`**, which is this "
            "specification's own and is the same on every build."
        ),
    )
    ...
    raw: dict[str, Any] | None = Field(
        default=None,
        description=(
            "The agent's own payload for this event, verbatim.\n\n"
            "**Shape differs per build and is not part of this contract** -- for "
            "debugging; depend on `content` and `type` instead. Present when "
            "`RunOptions.include_raw` asked for it, and it is the bulk of an "
            "event, so turn it off for transcript rendering."
        ),
    )
...
class RunResponse(BaseModel):
    ...
    result: str | None = Field(
        default=None,
        description=(
            "The agent's final answer as text, reassembled across the turn.\n\n"
            "**Null is a real state and not a gap**: a turn that was interrupted, "
            "ran out of wall clock, or ended without producing text has no "
            "result. That is why `is_error`, `interrupted` and `stop_kind` are "
            "separate fields -- do not infer any of them from this one being "
            "empty."
        ),
    )
    is_error: bool = Field(
    ...
    num_turns: int | None = Field(
        default=None,
        description=(
            "Agent turns inside this one request, as the agent counted them.\n\n"
            "**Null when the agent produced no result envelope** -- an "
            "interrupted or timed-out turn reports neither this nor "
            "`duration_ms`, which is a different thing from reporting zero. The "
            "unit is the agent's own, so it is a number to display rather than "
            "one to compare across builds."
        ),
    )
    ...
    duration_ms: int | None = Field(
        default=None,
        description=(
            "How long the turn took, in milliseconds, as the agent measured it.\n\n"
            "**The agent's clock, not this service's**, so it excludes whatever "
            "the HTTP layer spent around it and is null when no result envelope "
            "arrived. To bound a request use `RunOptions.timeout_s`, which this "
    ...
    permission_denials: list[Any] | None = Field(
        default=None,
        description=(
            "Tools the agent tried and was refused, as it reported them.\n\n"
            "**Where the cost of `allowed_tools` shows up**: an ungranted tool "
            "is still offered to the model, attempted, and denied -- a spent "
            "turn, recorded here. Shape is the agent's own; **null means the "
            "build reports no such list**, not that nothing was denied."
        ),
    )
    events: list[AgentEvent] = Field(
        default_factory=list,
        description=(
            "Every event of the turn, in `seq` order. **The streaming routes "
            "deliver the same objects one at a time instead.**\n\n"
            "Shaped by `include_partial_messages` (deltas or whole messages) and "
            "`include_raw` (whether each event carries the agent's own payload)."
        ),
    )
...
class Service(BaseModel):
    """Who is answering: the contract, the build, and the agent underneath.

    **Read `impl.version` to decide whether to re-read this payload**, and never
    as a cache key on its own -- `config` moves with the deployment rather than
    with the build.
    """
...
class Config(BaseModel):
    """How THIS instance is set up -- the operator's choices, not the build's.

    **Two containers of one image answer differently here**, which is the whole
    reason this payload is cached per instance and keyed by the base URL rather
    than by a version.
    """

    workspace_dir: str = Field(
        description=(
            "Absolute path **inside the container** that the agent works in, and "
            "what `RunOptions.working_directory` resolves against.\n\n"
            "**Mount a host directory here** or everything the agent writes dies "
            "with the container; `require_mounts` says whether this build refuses "
            "to start without one.\n\n"
            "Whatever ambient configuration that directory carries -- a memory "
            "file, a settings file -- is read on every turn."
        )
    )
    reference_dirs: list[str] = Field(
        description=(
            "Extra read-only directories the agent may read besides the "
            "workspace.\n\n"
            "**Empty means the build has no such mechanism wired** -- a fact "
            "about the build, not the deployment. No `RunOptions` field carries "
            "it, which is why it is published as `[]` rather than as a refusal "
            "nobody could trigger."
        )
    )
    default_model: str = Field(
        description=(
            "Model used when `RunOptions.model` is omitted.\n\n"
            "**Deployment-configured, not a property of the build** -- two "
            "containers of one image answer differently, so cache this payload "
            "per instance rather than against `impl.version`."
        )
    )
...
class Accepts(BaseModel):
    """What a caller may SEND: vocabularies, refusals and ceilings.

    **This group is exactly what `GET /v1/schemas/run-options` renders as JSON
    Schema.** The two are one fact in two shapes -- this one branchable, that one
    validatable -- and a conformance clause asserts they agree.
    """
    ...
    effort_levels: list[str] = Field(
        description=(
            "Reasoning levels this build delivers **exactly** -- a level it "
            "silently maps onto another is not listed.\n\n"
            "**Empty means `RunOptions.effort` is refused with a 400**: hide the "
            "control rather than rendering an empty dropdown."
        )
    )
    setting_sources: list[str] = Field(
        description=(
            "Ambient configuration layers a caller may ask this build to load -- "
            "`user`, `project`, `local` where they exist.\n\n"
            "**Empty means `RunOptions.setting_sources` is refused**, so nothing "
            "you send changes what that build reads from its disk.\n\n"
            "**A switch only.** Memory files, skills, subagents, commands and "
            "plugins are read from inside the container, and no field on any "
            "build can supply them."
        )
    )
    default_allowed_tools: list[str] = Field(
        description=(
            "What a session gets when `RunOptions.allowed_tools` is omitted -- "
            "**not the list of tool names that exist**, which is wider on every "
            "build.\n\n"
            "A starting selection, never a vocabulary to validate against: an "
            "unrecognised name is ignored rather than refused. **`[]` means the "
            "build governs by sandbox** and refuses `allowed_tools` outright."
        )
    )
    always_disallowed_tools: list[str] = Field(
        description=(
            "Tools removed from every session whatever a caller sends.\n\n"
            "**A filter, not a refusal**: naming one in `allowed_tools` is not an "
            "error -- it is dropped and the session proceeds, so this list is the "
            "only notice you get. Render them as disabled options."
        )
    )
    ...
    limits: dict[str, float] = Field(
        description=(
            "Ceilings and defaults on what a REQUEST may ask for, by name: "
            "`default_*` is what you get by omitting a field, `max_allowed_*` is "
            "the most you may send. **Above a ceiling is a 400, never a clamp.**\n\n"
            "**Free-form on purpose** -- a build publishes the ones it has, and a "
            "name absent here is a limit this build does not impose. Figures the "
            "build enforces on its own live in `behaviour.limits`; the split is "
            "why a key prefix is no longer the only clue which kind you are "
            "reading."
        )
    )
...
class Behaviour(BaseModel):
    """What this build DOES and REPORTS -- facts to design around, not to send.

    Nothing here constrains a request. It decides how a client reads a result,
    bills a turn, correlates a log line, or plans around a timeout.
    """

    limits: dict[str, float] = Field(
        description=(
            "Figures this build ENFORCES on its own, by name -- `session_idle_ttl_s` "
            "closes an idle session, `turn_timeout_s` ends a turn. **A promise "
            "about behaviour, not a constraint on your request**: nothing here is "
            "something you send, and nothing here answers a 400.\n\n"
            "Ceilings on what you may ask for are `accepts.limits`. A measured "
            "characteristic that nothing enforces is neither -- see "
            "`turn_token_overhead`."
        )
    )
    mcp_tool_call: McpToolCall = Field(
        description=(
            "What holds an MCP tool call open on this build, and what ends it. "
...
class Deployment(BaseModel):
    """Everything true of the service instance you are talking to.

    **Four groups because four different questions were being answered by one
    flat object**, and a reader could not tell which was which: a version, an
    operator's setting, a constraint on the request and a fact about behaviour
    all sat side by side. `limits` and `mcp` straddled the line and were cut.

    **Renamed from `Capabilities` at `/v1/capabilities`.** Half of what it
    carries was never a capability -- `workspace_dir` and `max_sessions` are
    configuration -- and the name misled accordingly.
    """
    ...
    #: Which group each published name belongs to. **The mapping lives here and
    #: nowhere else**: three builds would otherwise each carry their own copy of
    #: it, and a field added to one grouping and not another is the kind of
    #: divergence this payload exists to prevent.
    GROUPS: ClassVar[dict[str, tuple[str, ...]]] = {
        "service": tuple(Service.model_fields),
        "config": tuple(Config.model_fields),
        "accepts": tuple(Accepts.model_fields),
        "behaviour": tuple(Behaviour.model_fields),
    }

    @classmethod
    def from_flat(cls, **fields: Any) -> Deployment:
        """Build the grouped payload from the flat names a build already uses.

        **Deliberate, not a shim.** The alternative is each build distributing
        thirty-four fields into four groups by hand, three times, with nothing
        checking that they agree -- and the whole point of this shape is that a
        reader can tell which question a field answers. One mapping, one place,
        and a build keeps naming the facts it measured.

        `limits` and `mcp_tool_call` are the two that straddle: pass
        `accepts_limits` and `behaviour_limits` rather than one `limits`, because
        a ceiling on a request and a figure the service enforces are different
        claims that happened to share a map.
        """
        grouped: dict[str, dict[str, Any]] = {g: {} for g in cls.GROUPS}
        for name, value in fields.items():
            if name == "accepts_limits":
                grouped["accepts"]["limits"] = value
                continue
            if name == "behaviour_limits":
                grouped["behaviour"]["limits"] = value
                continue
            for group, members in cls.GROUPS.items():
                if name in members:
                    grouped[group][name] = value
                    break
            else:
                raise ValueError(
                    f"{name!r} belongs to no group of Deployment. Add it to one "
                    "of Service/Config/Accepts/Behaviour -- a published field "
                    "with no group is the flat payload this shape replaced."
                )
        return cls(**{g: globals()[g.title()](**v) for g, v in grouped.items()})
