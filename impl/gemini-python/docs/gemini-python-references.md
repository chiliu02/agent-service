...
## GP-66 — `system_prompt` was published, accepted and applied to nothing; it is now an env var and a file

**Measured 2026-09-02**, two turns on `gemini-3.1-flash-lite` plus one free
error, after a review found the field wired to nothing.

### The defect, stated plainly

`RunOptions.system_prompt` was in this build's published document, was **not** in
`unsupported_options`, and **no code path read it**. The build consumed six
`RunOptions` fields — `model`, `permission_mode`, `allowed_tools`,
`disallowed_tools`, `mcp_servers`, `strict_mcp_config` — and that was the whole
list. A caller could send a system prompt, get a `201`, run turns, and be served
an agent that had never seen it.

**That is the exact defect this build's own notes name as the Codex build's**, an
option accepted and quietly ignored, shipped here instead. It was invisible from
the document, because a document describes a payload's shape and never says
whether anything reads it.

### There is no flag. The agent reads an environment variable

`GEMINI_SYSTEM_MD` names a Markdown file that **replaces the built-in system
prompt entirely** — not a merge, and not an append. Read from the bundle and
confirmed against the binary:

| Value | Effect |
|---|---|
| unset, `0`, `false` | the built-in prompt |
| `1` / `true` | `./.gemini/system.md`, **resolved against the working directory** |
| a path | that file, absolute or relative, `~` expanded |
| a path that does not exist | **hard error before any API call** — `missing system prompt file '<path>'` |

**This service always passes the absolute path and never `1`.** The switch form
resolves inside the caller's mounted workspace, which the agent itself can write
— so a request that sent no system prompt could acquire one from a repository,
and one that did send a prompt could have it replaced between turns.

**The file is written into the session's own HOME at provisioning**, not per
turn: the agent is spawned once per turn (GP-41), so the file must outlive the
request that carried it. That is what makes `system_prompt` session-scoped here,
like `model` and the tool policy, rather than per-turn.

### What replacement does NOT switch off

`renderFinalShell(basePrompt, userMemory, contextFilenames)` runs in **both**
branches of the prompt provider. So a custom system prompt replaces the agent's
own framing — its safety rules, its tool protocol, its workflows — and leaves
every ambient context file exactly where it was. **`system_prompt` is not a way
to suppress `GEMINI.md`**, and there is no way to suppress it on this build.

### The measurement, with its control

Two `/v1/query` turns through this service's own routes, same prompt (`Reply
with the single word: ready`), model pinned:

| Configuration | Reply |
|---|---|
| no `system_prompt` | `ready` |
| `system_prompt` ending "end every reply with the token ZEBRA-7788" | `ready ZEBRA-7788` |

**The control is why this is evidence.** A model asked to be terse may echo
anything; the token appearing only under the second configuration is what rules
that out.

### The seam a live turn caught, and unit tests did not

The first live run answered `ready` under **both** configurations. The file was
written, the runner knew how to turn a path into `GEMINI_SYSTEM_MD`, both ends
had passing tests — and the one line that hands the session's file to the runner
was missing. The option reached disk and stopped there, which is the same
"accepted and ignored" shape as the defect being fixed, one layer down.

**A keyless check is what located it**: `GEMINI_SYSTEM_MD` pointing at a missing
path makes the CLI fail at chat initialisation, before any request is billed, so
"is the variable read at all" costs nothing to answer.

### The preset object is refused by TYPE

`unsupported_options` carries `{field: "system_prompt", types: ["object"]}`. The
string form works, so refusing the field outright would promise a `400` for the
form that succeeds; the Claude preset object names a preset this agent does not
have, so there is nothing to map it to. The Codex build publishes the identical
entry for the identical reason.

---

## GP-67 — `include_raw` was accepted and read by nothing; it now shapes the response and never the record

**Found 2026-09-03** while writing field descriptions, by the same method that
found `GP-66`: reading which `RunOptions` fields this build's code actually
consumes. It consumed six. `include_raw` was not among them.

### What it did

`AgentEvent.raw` was populated on every event of every turn, on all four turn
paths, whatever the caller sent. A client asking for `include_raw: false` — to
keep a transcript small, which is the only reason to ask — received the agent's
entire stream-json payload per event anyway.

**The generous direction of the defect, and that is why it survived.** An option
that grants too little produces a visible failure; one that grants too much
produces a bigger response nobody inspects. The other two builds honour the
field: the Claude build threads it into its normaliser, and the Codex build reads
`getattr(options, "include_raw", None) is not False`.

### What it does now

Session-scoped, like every option on this build — a turn here carries a prompt
and nothing else, so there is nowhere later to change it.

| | `raw` |
|---|---|
| response and SSE frames | dropped when `include_raw` is `false` |
| the recorded copy (`events` rows in the database) | **kept, always** |

**The split is deliberate.** History belongs to the operator, and a
response-shaping option must not delete data from storage: a caller who wanted a
smaller payload has not asked for the transcript to be lossy, and nothing in the
field says otherwise.

### Both directions are tested

`false` asserts every event's `raw` is null; `true` asserts at least one is not.
The one-directional version of this test passes against the defect.

---

## GP-68 — `workspace_subdir` was accepted and read by nothing; here it is a boundary as well as a cwd

**Found 2026-09-03**, the third inert field on this build in two days, by the
same method as `GP-66` and `GP-67`: reading which `RunOptions` fields the code
actually consumes. A search for `subdir` across `src/` returned nothing.

Every session started at the workspace root, whatever the caller sent.

### What it does now

`resolve_workspace()` in `registry.py`: the root when omitted, an existing
directory under the root when named, and a **400** otherwise -- outside the root,
or missing. The directory is never created; it is caller-supplied per request
and creating one would litter the caller's mounted workspace.

The resolved path becomes the session's `cwd` and the runner spawns the agent
there.

### On this build it is also a BOUNDARY, which it is not on the other two

The agent's own guard refuses a file tool that wanders outside the directory it
was started in -- *"Path not in workspace: attempted path resolves outside the
allowed workspace directories"*. So a subdirectory **narrows what the agent can
reach**, where the Claude build's `cwd` restricts nothing (its `add_dirs` and cwd
are not confinement) and the Codex build's boundary is bubblewrap over the
workspace root.

**That is a difference to publish rather than smooth over**: the same field on
three builds means *where it starts* everywhere, and *what it may touch* here.

### And it anchors ambient discovery

`GEMINI.md` and a workspace `.gemini/settings.json` are found relative to the
working directory (GP-46), so this value also chooses which project
configuration the session picks up -- on the build that refuses
`setting_sources` and therefore has no other lever over ambient input at all.

### Tested with the control in both directions

An existing `sub/` must open a session; `../../etc` and a missing directory must
both be 400. Plus the seam -- `_runner_for` must hand the resolved path to the
runner, which is exactly the join `GP-66` was broken at.
