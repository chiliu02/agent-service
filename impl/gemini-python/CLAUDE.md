...
- `docs/gemini-python-guide.md` — **what a client author reads first**: the
  boot gates, the ten things that will surprise them, the capability table
  and what stays their responsibility.
- `docs/gemini-python-references.md` — **52 entries, `GP-01`–`GP-51`**, seeded
...
- `spike/` — **nine probes**, committed on purpose as evidence:
  - **free entirely** — `probe_gemini_cli.py`, which answers everything the
    ...
    alone;
  - **free half, paid half** — `probe_gemini_mcp_live.py` and
    `probe_gemini_modes_live.py` run their keyless checks and stop when
    `GEMINI_API_KEY` is unset, which is how the policy-validation findings stay
    reproducible for nothing;
