...
from agent_service.registry import Registry, RegistryFull, Session, UnknownSession
...
#:
#: **WHOLE-FIELD entries only.** An entry carrying `values` refuses particular
#: values and honours the rest, so refusing the field here would 400 the value
#: that works -- `strict_mcp_config: true` is what this build actually does. Such
#: an entry is enforced by the code that owns the condition (`_mcp_request` for
#: that one), which also answers with the problem `type` naming what failed
#: rather than the generic one: the field is supported and the VALUE is not.
_REFUSED_FIELDS = frozenset(o.field for o in UNSUPPORTED_OPTIONS if o.values is None)


def _refused_option(options: Any) -> str | None:
    ...
    **`exclude_unset`, so an absent field is not a request.** A client that
    ...
    for field in sorted(_REFUSED_FIELDS):
        if sent.get(field) not in (None, [], {}, ()):
            return field
    return None
...
def _unsupported(field: str) -> JSONResponse:
    return problem(
        400, "unsupported-options", f"This build does not support {field!r}",
        f"capabilities.unsupported_options names {field!r}. It is refused rather "
        "than accepted and ignored: an option that is taken and never applied is "
...
def _agent_event(seq: int, event: dict[str, Any]) -> AgentEvent:
    return AgentEvent(seq=seq, type=_event_type(event), subtype=_subtype(event),
                      content=_content_of(event), raw=event)
