...
class CliRunner:
    ...
    def __init__(
        ...
        *,
        ...
        #: **`RunOptions.system_prompt`, and it is an ENVIRONMENT variable
        #: rather than a flag** (GP-66). There is no `--system-prompt`: the
        #: agent reads `GEMINI_SYSTEM_MD`, and an absolute path is what makes
        #: the file ours rather than something resolved against the caller's
        #: workspace. `None` leaves the variable unset, which is the built-in
        #: prompt.
    ...
    def env(self) -> dict[str, str]:
        ...
        if self._system_prompt_file is not None:
            # **The absolute path, never `1`** (GP-66). The switch form resolves
            # its project-default file against the working directory, which is the
            # caller's mounted workspace -- so the agent would read a file the
            # agent itself can write, and a request that sent no system prompt
