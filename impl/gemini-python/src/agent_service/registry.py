...
def resolve_workspace(root: Path, subdir: str | None) -> Path:
    ...
    **This build read the field not at all until 2026-09-03** (GP-68) -- it was
    accepted, and every session started at the workspace root.

    **Here the working directory is also a boundary, which it is not on the
    other builds.** The agent's own guard refuses a file tool that wanders
    outside the directory it was started in, so a subdirectory narrows what the
    agent can reach as well as where it begins. That is a difference to publish
    rather than smooth over.

    **It also anchors ambient discovery**: the agent's context files and a workspace
    `.gemini/settings.json` are found relative to this directory (GP-46), so the
    value chooses which project configuration the session picks up.
...
class Session:
    ...
    #: **The caller's `system_prompt`, on disk, because the agent takes a FILE**
    #: (GP-66). `None` is the built-in prompt. It sits in this session's own
    #: HOME rather than the workspace: the workspace is one directory shared by
    #: every session and writable by the agent, so a prompt written there would
    #: be readable and rewritable by the next turn.
    system_prompt_file: Path | None = None
    #: **`RunOptions.include_raw`, which this build accepted and ignored until
    #: 2026-09-03** (GP-67). Session-scoped like every option here, because a
    #: turn on this build carries a prompt and nothing else. It shapes what the
    #: CALLER receives; the recorded copy keeps the payload either way.
    include_raw: bool = True
    #: **Where the agent STARTS, and on this build also what it may reach**
    #: (GP-68). Defaults to the workspace root; `RunOptions.working_directory`
    #: narrows it. The agent's own guard is anchored here, so a subdirectory
    #: confines its file tools as well as choosing its cwd.
    cwd: Path | None = None
...
class Registry:
    ...
    def _provision(
        ...
        **Three coupled pieces** (GP-46, GP-47): the servers go in the session's
        ...
        if system_prompt:
            # **Written once, at provisioning, and read on every turn** (GP-66).
            # The agent resolves `GEMINI_SYSTEM_MD` per invocation and this
            # build spawns per turn (GP-41), so the file has to outlive the
            # request that carried it -- which is what makes `system_prompt` a
            # session-scoped option here rather than a per-turn one.
            prompt_file = session.agent_home / "system.md"
            prompt_file.write_text(system_prompt, encoding="utf-8")
            session.system_prompt_file = prompt_file
    ...
    def create(
        ...
        *,
        ...
        self._provision(session, allowed_tools=allowed_tools,
                        ...
                        mcp_servers=mcp_servers, system_prompt=system_prompt)
