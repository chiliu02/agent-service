"""The meta surface: `/healthz`, `/v1/capabilities`, and the boot gates.
...
def test_a_supplied_sdk_session_id_is_refused(client: TestClient) -> None:
    ...
    """
    assert client.get("/v1/capabilities").json()["allow_supplied_sdk_session_id"] is False
...
def test_cost_cannot_be_promised_so_a_budget_is_refused(client: TestClient) -> None:
    ...
    """
    refused = {option["field"] for option in client.get("/v1/capabilities").json()["unsupported_options"]}
...
def test_the_enforced_limits_are_promises_not_a_config_dump(client: TestClient) -> None:
    ...
    """
    limits = client.get("/v1/capabilities").json()["limits"]
...
def test_permission_enforcement_is_none_and_that_is_not_no_boundary(
    ...
    """
    assert build_capabilities(_settings(tmp_path)).permission_enforcement == "none"
