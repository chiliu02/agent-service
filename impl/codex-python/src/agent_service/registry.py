...
class SessionRegistry:
    ...
    def _default_factory(self, options: Any, working_directory: str | None) -> CodexSession:
        # **Resolved and checked, not concatenated** (CX-64). Until 2026-09-03
        # this was `cwd / subdir` with no containment and no existence check, so
        # `../..` started the thread outside the workspace and a missing
        # directory failed later inside the agent. It also decides which
        # `AGENTS.md` the thread reads (CX-14), so it moves ambient
        # configuration as well as the working directory.
        cwd = resolve_workspace(self._settings, working_directory)
        return CodexSession(
            cwd=str(cwd),
    ...
    async def create(
        ...
        **This is the one chokepoint for options on this build**, which is why
        ...
        session = self._factory(options, getattr(options, "working_directory", None))
        try:
