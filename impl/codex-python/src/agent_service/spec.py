...
def specification() -> dict[str, object]:
    ...
    try:
        build_version = version("agent-service-codex-python")
    except PackageNotFoundError:  # pragma: no cover - only outside an install
        build_version = None
    return {
        "version": build_version,
        "document_version": DOCUMENT_VERSION,
        # **The OTHER thing this image is built against** (CX-58). An image
        ...
        # The agent target this build drives -- `codex`, the family name
        # without the language suffix (CX-57). A consumer relaying to a vendor
        # maps it: `codex` is the OpenAI API.
        "model_api": MODEL_API,
        "provider_selectors": list(PROVIDER_SELECTOR_ENV_VARS),
        # **TRUE, and the consumer was told it would be `false`.** That reply was
        # written on 2026-08-08, when this build read `AGENT_SERVICE_AUTH_TOKEN`,
        # published `auth_required` from it and checked no request against it --
        # so it refused to boot rather than claim a protection it lacked. Bearer
        ...
        # it and translates it into the config-override route that does work. The
        # consumer guessed this name; it is true by implementation rather than by
        # luck. `config.endpoint_overrides` is the translation.
