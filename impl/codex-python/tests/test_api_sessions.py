...
def test_an_operator_can_still_forbid_mcp_entirely(tmp_path) -> None:  # noqa: ANN001
    ...
    **The reason is attribution rather than capability**, exactly as on the
    ...
    with TestClient(create_app(settings, SessionRegistry(settings))) as c:
        assert flat(c.get("/v1/deployment").json())["allow_mcp_servers"] is False
...
def test_the_published_transports_are_the_ones_accepted(client) -> None:  # noqa: ANN001
    ...
    """
    mcp = flat(client.get("/v1/deployment").json())["mcp"]
...
def test_every_published_unsupported_option_really_is_refused(client) -> None:  # noqa: ANN001
    ...
    another, so this drives the request from `/v1/deployment` rather than from
...
def test_the_published_cap_is_the_enforced_cap(client) -> None:  # noqa: ANN001
    ...
    cap = flat(client.get("/v1/deployment").json())["max_sessions"]
    for _ in range(cap):
        assert client.post("/v1/sessions", json={}).status_code == 201
...
def test_no_bound_on_a_tool_call_is_published_as_null_not_omitted(client) -> None:  # noqa: ANN001
    ...
    """
    tool_call = flat(client.get("/v1/deployment").json())["mcp_tool_call"]
...
def test_working_directory_must_stay_under_the_root(client: TestClient) -> None:
    """CX-64. It was concatenated with no checks at all until 2026-09-03.

    **`..` walked out of the workspace**, and because the value also decides
    which `AGENTS.md` the thread reads (CX-14), an escape moved the agent's
    ambient configuration as well as its working directory.
    """
    escaped = client.post(
        "/v1/sessions", json={"options": {"working_directory": "../../etc"}}
    )
    assert escaped.status_code == 400
    assert escaped.headers["content-type"].startswith("application/problem+json")
    assert "working_directory" in escaped.json()["detail"]


def test_working_directory_must_already_exist(client: TestClient) -> None:
    """CX-64. Not created from request input, and refused early rather than late.

    A syntactically valid, in-root, nonexistent directory used to sail through
    and fail opaquely inside the agent -- where the caller cannot tell a missing
    directory from a broken agent.
    """
    missing = client.post(
...
def test_an_existing_subdir_is_accepted_and_becomes_the_cwd(
    client: TestClient, tmp_path,  # noqa: ANN001
) -> None:
    """The control: the refusals above must not be refusing everything."""
    (tmp_path / "ws" / "sub").mkdir(parents=True)
    created = client.post(
        "/v1/sessions", json={"options": {"working_directory": "sub"}}
    )
