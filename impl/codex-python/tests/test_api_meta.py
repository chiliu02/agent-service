...
import pytest
from agent_spec.openapi.schemas import Capabilities, Health
...
# `/v1/capabilities`. Publishing a gate that does not fire is worse than not
...
def test_v1_without_a_credential_is_401_as_a_problem_document(
    ...
    r = authed_client.get("/v1/capabilities")
...
def test_the_right_token_is_let_through(authed_client: TestClient) -> None:
    r = authed_client.get("/v1/capabilities", headers={"Authorization": "Bearer s3cret"})
...
def test_a_wrong_token_and_a_missing_one_are_indistinguishable(
    ...
    missing = authed_client.get("/v1/capabilities")
    wrong = authed_client.get("/v1/capabilities", headers={"Authorization": "Bearer nope"})
...
def test_the_token_never_appears_in_any_response(authed_client: TestClient) -> None:
    for r in (
        authed_client.get("/v1/capabilities"),
        authed_client.get("/v1/capabilities", headers={"Authorization": "Bearer nope"}),
...
def test_another_scheme_is_refused_and_named(authed_client: TestClient) -> None:
    r = authed_client.get("/v1/capabilities", headers={"Authorization": "Basic s3cret"})
    assert r.status_code == 401
...
def test_capabilities_validates_against_the_shared_model(client: TestClient) -> None:
    Capabilities.model_validate(client.get("/v1/capabilities").json())
...
def test_the_sandbox_capability_matches_what_was_measured(client: TestClient) -> None:
    ...
    """
    sandbox = client.get("/v1/capabilities").json()["sandbox"]
...
def test_the_published_example_is_what_a_live_instance_actually_answers() -> None:
    ...
    with TestClient(create_app(_OPEN)) as client:
        ...
        live = client.get("/v1/capabilities").json()
