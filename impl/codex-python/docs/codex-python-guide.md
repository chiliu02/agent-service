...
`GET /v1/deployment`, and every one of them measured rather than assumed.
...
`GET /v1/deployment` — §4 is the list for this build.
...
`deployment.behaviour.sandbox` reports `network_access: false` and
...
Read the value from `deployment.config.default_model` rather than assuming either
one.
...
**7. Read `/v1/deployment` rather than assuming.** §4 is a table of ten
