...
def published_spec() -> dict[str, Any]:
    ...
    impl = (httpx.get(f"{url}/v1/deployment", timeout=30.0)
            .json()["service"]["impl"]["name"])
...
def capabilities(base_url: str) -> dict[str, Any]:
    """`GET /v1/deployment` from the service under test, FLATTENED.
    ...
    **The join between an optional clause and the build in front of you.**
    ...
    # **Flattened here so every conditional clause keeps reading one mapping.**
    # Which group holds a field is asserted by the clauses that own the shape;
    # a clause about whether a build ADOPTS a supplied id should not also have
    # to know that the answer lives under `accepts`.
    return flat(httpx.get(f"{base_url}/v1/deployment", timeout=30.0).json())
    ...
    **Read, never defaulted.** A missing field is a build older than 0.18.0 or
    ...
    if not isinstance(value, bool):
        pytest.fail(
            ...
            "on /v1/deployment. AS-13 is conditional on it since 0.18.0, so "
