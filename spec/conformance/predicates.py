...
#: The four groups of `GET /v1/deployment`, named here rather than imported.
#:
#: **The suite must not import `agent_spec`.** It judges a build from outside,
#: over HTTP, and a suite that borrowed the producer's own constants could not
#: catch a producer that changed them -- which is the whole point of a
#: conformance suite being a separate artifact.
DEPLOYMENT_GROUPS: tuple[str, ...] = ("service", "config", "accepts", "behaviour")


def flat(payload: dict[str, Any]) -> dict[str, Any]:
    """The grouped payload as one mapping, for checks about a field's VALUE.

    Which group holds a field is asserted once, by the clause that owns the
    shape; every other check reads through here so that moving a field between
    groups is one edit rather than fifty.

    `limits` lives in two groups, so colliding maps are merged rather than
    overwritten -- dropping half of it silently is exactly the failure this
...
def _deployment_properties(spec: Spec, group: str, clause: str) -> dict[str, Any]:
    """One group's properties, resolved through `Deployment`'s `$ref`.

    **The payload is grouped since 2026-09-03**, so a clause that used to read
    `Capabilities.properties` now names the group it means. That is the point of
    the change rather than a cost of it: a clause saying *`accepts` publishes
    this* is saying which question the field answers.
    """
    schemas = spec.get("components", {}).get("schemas", {})
    # **The clause id travels with the message.** A predicate that fails without
    # naming its clause is unactionable, and the negative control asserts as
    # much -- a document missing the whole component must still say which rule
    # noticed.
    assert "Deployment" in schemas, (
        f"{clause}: the document publishes no Deployment component, so nothing "
        "describes GET /v1/deployment"
    )
    ref = (schemas["Deployment"].get("properties") or {}).get(group, {}).get("$ref", "")
    assert ref, f"{clause}: Deployment does not carry a {group!r} group"
    return _properties(spec, ref.rsplit("/", 1)[-1])
...
def as1_capabilities_publishes_two_separate_arrays(spec: Spec) -> None:
    ...
    **`config`, because both name what the OPERATOR set up**: which environment
    variables this instance reads a credential from, and which select a
    provider. A caller sends neither.
    """
    props = _deployment_properties(spec, "config", "AS-1")
    for field in ("credential_sources", "provider_selectors"):
        assert field in props, f"AS-1: deployment.config does not publish {field}"
        assert props[field].get("type") == "array", (
            f"AS-1: config.{field} is not an array"
        )
    required = _schema(spec, "Config").get("required", [])
    for field in ("credential_sources", "provider_selectors"):
        assert field in required, (
            f"AS-1: config.{field} is optional, so a response may omit it"
        )
...
def as5_capabilities_publishes_the_cap_and_the_boot_gates(spec: Spec) -> None:
    ...
    **All three are `config`**: they describe how this instance was started, not
    what a caller may send. That is the distinction the grouping exists for.
    """
    props = _deployment_properties(spec, "config", "AS-5")
    for field in ("max_sessions", "require_credentials", "require_mounts"):
        assert field in props, f"AS-5: deployment does not publish {field}"
...
def as11_the_one_shot_routes_carry_the_id_in_the_body(spec: Spec) -> None:
    ...
    **What replaces it is a capability, not a relaxation.** The header's presence
    on that route is governed by `deployment.behaviour.query_reports_sdk_session_id`
    ...
    **A document tier cannot check the condition itself** -- the capability's
    ...
    capabilities = _deployment_properties(spec, "behaviour", "AS-11/AS-32")
    assert "query_reports_sdk_session_id" in capabilities, (
        "AS-11/AS-32: deployment.behaviour does not publish query_reports_sdk_session_id, "
